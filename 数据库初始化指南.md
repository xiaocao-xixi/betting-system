# 🚨 数据库初始化指南 | Database Initialization Guide

## 问题症状 | Problem Symptoms

如果您看到以下任何错误：
If you see any of these errors:

- `users.map is not a function`
- `Runtime TypeError`
- 页面显示错误信息 | Page shows error message
- "没有用户数据" | "No user data"

**这意味着数据库还没有初始化！**
**This means the database hasn't been initialized yet!**

---

## ✅ 完整解决方案 | Complete Solution

### 第 1 步：确保在项目目录 | Step 1: Ensure in project directory

```bash
cd betting-system
# 或者您的项目路径 | Or your project path
```

### 第 2 步：创建 .env 文件 | Step 2: Create .env file

如果 `.env` 文件不存在，创建它：
If `.env` file doesn't exist, create it:

```bash
# Windows
copy .env.example .env

# Mac/Linux
cp .env.example .env
```

`.env` 文件内容应该是：
The `.env` file should contain:

```
DATABASE_URL="file:./dev.db"
NODE_ENV="development"
```

### 第 3 步：运行数据库迁移 | Step 3: Run database migration

```bash
npx prisma migrate dev --name init
```

**预期输出 | Expected output:**
```
✔ Generated Prisma Client
✔ Database migrations applied
```

### 第 4 步：生成 Prisma 客户端 | Step 4: Generate Prisma client

```bash
npx prisma generate
```

**预期输出 | Expected output:**
```
✔ Generated Prisma Client
```

### 第 5 步：填充测试数据 | Step 5: Seed test data

```bash
npm run prisma:seed
```

**预期输出 | Expected output:**
```
Seeding database...
Created 10 users with initial balances
✅ Seed completed successfully!
```

这将创建 10 个测试用户，每个用户有 1000 的初始余额。
This creates 10 test users, each with an initial balance of 1000.

### 第 6 步：重启开发服务器 | Step 6: Restart dev server

```bash
# 停止当前服务器 (Ctrl+C)
# Stop current server (Ctrl+C)

# 重新启动
npm run dev
```

### 第 7 步：刷新浏览器 | Step 7: Refresh browser

访问 `http://localhost:3000` 并刷新页面。

Visit `http://localhost:3000` and refresh the page.

---

## 🔍 验证数据库 | Verify Database

### 检查数据库文件是否存在 | Check if database file exists

```bash
# Windows
dir *.db

# Mac/Linux
ls -la *.db
```

应该看到 `dev.db` 文件。
You should see a `dev.db` file.

### 检查用户数据 | Check user data

```bash
npx prisma studio
```

这将打开 Prisma Studio，您可以在浏览器中查看数据库内容。
This opens Prisma Studio where you can view database contents in the browser.

---

## 🆘 常见问题 | Common Issues

### 问题 1: "prisma command not found"

**解决方案 | Solution:**
```bash
npm install
```

### 问题 2: "Environment variable not found: DATABASE_URL"

**解决方案 | Solution:**

确保 `.env` 文件存在且包含：
Make sure `.env` file exists and contains:
```
DATABASE_URL="file:./dev.db"
```

### 问题 3: 迁移失败 "Migration failed"

**解决方案 | Solution:**

删除数据库并重新开始：
Delete database and start over:

```bash
# Windows
del dev.db
rmdir /s /q prisma\migrations

# Mac/Linux
rm -f dev.db
rm -rf prisma/migrations

# 然后重新运行迁移 | Then run migration again
npx prisma migrate dev --name init
```

### 问题 4: "种子脚本失败" | "Seed script failed"

**解决方案 | Solution:**

检查数据库是否已迁移：
Check if database is migrated:

```bash
npx prisma migrate status
```

如果没有迁移，运行：
If not migrated, run:

```bash
npx prisma migrate dev
```

---

## 📝 快速命令参考 | Quick Command Reference

```bash
# 完整的初始化流程 | Complete initialization flow
npx prisma migrate dev --name init
npx prisma generate
npm run prisma:seed
npm run dev

# 重置数据库 | Reset database
npx prisma migrate reset

# 查看数据库 | View database
npx prisma studio

# 查看迁移状态 | Check migration status
npx prisma migrate status
```

---

## 🎯 一键设置脚本 | One-Click Setup Script

使用自动设置脚本（推荐）：
Use auto-setup script (recommended):

```bash
# Windows
verify-setup.bat

# Mac/Linux
./verify-setup.sh
```

这些脚本会自动完成所有步骤！
These scripts automatically complete all steps!

---

## 🔗 相关文档 | Related Documentation

- [START_HERE.md](./START_HERE.md) - 快速入门
- [DEPLOYMENT_GUIDE.md](./DEPLOYMENT_GUIDE.md) - 部署指南
- [TROUBLESHOOTING.md](./TROUBLESHOOTING.md) - 故障排除

---

**完成后，您应该能看到包含 10 个用户的列表！** ✅

**After completion, you should see a list with 10 users!** ✅
